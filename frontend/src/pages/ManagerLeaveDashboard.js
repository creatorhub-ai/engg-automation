// ManagerLeaveDashboard.js - COMPLETE WORKING VERSION
...
function ManagerLeaveDashboard({ user, token }) {
  ...
  // NEW: Modal state for trainer modules
  const [moduleDialogOpen, setModuleDialogOpen] = useState(false);
  const [selectedTrainerModules, setSelectedTrainerModules] = useState([]);
  ...
  // Fetch modules for specific dates
  const fetchModulesForDates = async (dates) => {
    try {
      const uniqueDates = [...new Set(dates.map(formatDate))];
      const missingDates = uniqueDates.filter((date) => !modules[date]);
  ...
  // NEW: Fetch ALL modules for selected trainer's leave dates
  const fetchTrainerModules = async (trainerReq) => {
    try {
      const start = new Date(trainerReq.start_date);
      const end = new Date(trainerReq.end_date || trainerReq.start_date);
      const dates = [];
      ...
      await fetchModulesForDates(dates);
      
      // Collect unique modules across all leave dates
      const allModules = new Set();
      dates.forEach(date => {
        const dateStr = formatDate(date);
        if (modules[dateStr]) {
          modules[dateStr].forEach(module => allModules.add(module));
        }
      });
      
      setSelectedTrainerModules(Array.from(allModules));
      setModuleDialogOpen(true);
    } catch (err) {
      console.error('Failed to fetch trainer modules:', err);
    }
  };
  ...
  const dayEventsMap = useMemo(() => {
    ...
    filteredRequests.forEach((req) => {
      ...
      while (cursorDate <= end) {
        ...
        map[key].push({
          ...
          domain: req.domain,
          reason: req.reason,
          modules: Array.from(dateModules),
          request: req, // NEW: Store full request for click handler
          category,
        });
    ...
    holidays.forEach((h) => {
      ...
      map[key].push({
        ...
        domain: "",
        reason: h.name,
        modules: modules[key] ? Array.from(modules[key]) : [],
        category,
      });
    });
  ...
  // NEW: Handle trainer chip click
  const handleTrainerClick = async (eventData) => {
    if (eventData.category === "trainer" && eventData.request) {
      await fetchTrainerModules(eventData.request);
    }
  };
  ...
  // UPDATED: Clickable trainer chips
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
        {events.map((ev) => {
          ...
          if (ev.category === "holiday") {
            ...
            isClickable = ev.category === "trainer";
          }
              ...
              {isClickable ? (
                <Chip
                  ...
                  sx={{
                    ...
                    cursor: "pointer",
                    "&:hover": {
                      bgcolor: `${chipBg}CC`, // Slightly darker on hover
                    },
                  }}
                />
              ) : (
  ...
  const ModuleDialog = () => (
    ...
    >
      <DialogTitle>
        📚 Modules Missed During Leave
      </DialogTitle>
      <DialogContent>
        {selectedTrainerModules.length > 0 ? (
          <List>
            {selectedTrainerModules.map((module, index) => (
              <ListItem key={index}>
                <ListItemIcon>
          ...
          <Typography color="text.secondary">
            No modules found for these leave dates.
          </Typography>
        )}
      </DialogContent>
    </Dialog>
  ...
  const renderMonthView = () => {
    ...
    return (
      <>
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", borderBottom: "1px solid #e0e0e0", pb: 1, mb: 1 }}>
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
            <Box key={d} sx={{ textAlign: "center", fontWeight: "bold", color: blue[800], fontSize: 13 }}>
              {d}
            </Box>
          ))}
        </Box>
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 0.5 }}>
          {weeks.map((week, wi) => week.map((day, di) => (
            ...
            >
              <Typography variant="caption" sx={{ fontWeight: "bold", mb: 0.5, textAlign: "right" }}>
                {day.displayDay}
              </Typography>
  ...
  const renderWeekView = () => {
    ...
    return (
      <>
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", borderBottom: "1px solid #e0e0e0", pb: 1, mb: 1 }}>
          {days.map((d) => (
            <Box key={d.toISOString()} sx={{ textAlign: "center", fontWeight: "bold", color: blue[800], fontSize: 13 }}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 0.5 }}>
          {days.map((d) => {
            ...
            if (hasHoliday) { bgColor = green[50]; borderColor = green[200]; }
            else if (hasOptional) { bgColor = red[50]; borderColor = red[200]; }
            else if (hasTrainerLeave) { bgColor = deepPurple[50]; borderColor = deepPurple[200]; }

            return (
              <Box key={key} sx={{ minHeight: 120, borderRadius: 1, border: `1px solid ${borderColor}`, bgcolor: bgColor, p: 0.75, display: "flex", flexDirection: "column" }}>
  ...
  const renderDayView = () => {
    ...
    return (
      <Box sx={{ borderRadius: 1, border: "1px solid #e0e0e0", p: 1.5, minHeight: 150 }}>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", mb: 2, justifyContent: "space-between", flexWrap: "wrap", gap: 2 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Typography variant="h6" fontWeight="bold">Trainer Leave Calendar</Typography>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <IconButton size="small" onClick={goPrev}><ArrowBackIosNewIcon fontSize="small" /></IconButton>
            <Typography variant="subtitle1" fontWeight="medium">
              {viewType === "month" ? monthLabel : cursor.toLocaleDateString("default", { day: "numeric", month: "long", year: "numeric" })}
            </Typography>
            <IconButton size="small" onClick={goNext}><ArrowForwardIosIcon fontSize="small" /></IconButton>
          </Box>

          <FormControl size="small" sx={{ minWidth: 200 }}>
            <InputLabel>Trainer</InputLabel>
            <Select label="Trainer" value={selectedTrainerId} onChange={(e) => setSelectedTrainerId(e.target.value)}>
              <MenuItem value="all"><em>All Trainers</em></MenuItem>
              {trainers.map((t) => {
                const key = t.email?.toLowerCase() || `id:${t.trainer_id || t.id}`;
                return <MenuItem key={key} value={key}>{t.name || t.trainer_name} ({t.email})</MenuItem>;
              })}
            </Select>
          </FormControl>
        </Box>

        {/* Holiday upload and legend remain the same */}
        <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2, flexWrap: "wrap" }}>
          <Typography variant="body2" sx={{ fontWeight: "bold" }}>Upload Holiday List (updates holidays table)</Typography>
          <input type="file" accept=".xlsx,.xls,.csv" onChange={handleHolidayFileChange} style={{ maxWidth: 260 }} />
          <Button variant="contained" size="small" onClick={handleHolidayUpload} disabled={!holidayFile}>Upload</Button>
        </Box>
        ...
        <Box sx={{ display: "flex", gap: 2, mb: 2, flexWrap: "wrap", fontSize: 14 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Box sx={{ width: 16, height: 16, bgcolor: red[50], border: `1px solid ${red[200]}` }} />
            <Typography variant="body2">Restricted / Optional Holiday</Typography>
          </Box>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Box sx={{ width: 16, height: 16, bgcolor: green[50], border: `1px solid ${green[200]}` }} />
            <Typography variant="body2">Holiday</Typography>
          </Box>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
            <Box sx={{ width: 16, height: 16, bgcolor: deepPurple[50], border: `1px solid ${deepPurple[200]}` }} />
            <Typography variant="body2">Trainer Leave (click to see modules)</Typography>
          </Box>
        </Box>
