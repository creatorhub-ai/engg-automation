...
import {
  ...
  CircularProgress, Alert, Grid, Card, CardContent, Chip, Skeleton, Fade
} from "@mui/material";
...
export default function AttendanceReport({ user, token }) {
  ...
  useEffect(() => {
    const fetchBatches = async () => {
      try {
        ...
        const fallback = ["PD53", "PDFT16", "PDFT15", "PDFT17", "DFT36", "DV22", "PD54"];
        setBatches(fallback);
        setBatchNo(fallback[0]);
      }
    };
    fetchBatches();
  }, []);
  ...
  useEffect(() => {
    fetchAttendance(batchNo);
  }, [batchNo, fetchAttendance]);
  ...
  const summary = useMemo(() => {
    ...
    const effectiveTotalSessions = totalSessions > 0 ? totalSessions : learners.length || 1;
    
    const avgAttendance = learners.length > 0
      ? learners.reduce((sum, l) => sum + (l.present / effectiveTotalSessions * 100), 0) / learners.length
      : 0;

    return {
      learners: learners.sort((a, b) => {
        const pctA = (b.present / effectiveTotalSessions) * 100;
        const pctB = (a.present / effectiveTotalSessions) * 100;
        return pctB - pctA;
      }),
  ...
  const downloadPDF = useCallback(() => {
    ...
    doc.text(`${summary.avgAttendance}% (${summary.totalLearners} learners)`, 14, 40);

    const tableData = summary.learners.map((l, i) => {
      const pct = summary.totalSessions > 0 ? (l.present / summary.totalSessions * 100) : 0;
      return [i+1, l.name.slice(0,20), l.email.slice(0,25), summary.totalSessions, l.present, l.leave, l.absent, pct.toFixed(1)];
    });
    ...
    doc.save(`attendance_${batchNo}_${Date.now()}.pdf`);
  }, [batchNo, summary]);
                    ...
                    <CardContent>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        ...
                        <Box>
                          ...
                          <Typography color="text.secondary">
                            Average Attendance ({summary.totalSessions} sessions)
                          </Typography>
                        </Box>
                      </Box>
                    </CardContent>
