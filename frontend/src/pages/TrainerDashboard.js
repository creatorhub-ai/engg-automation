...
function TrainerUnavailabilityForm({ user }) {
  ...
  return (
    <Paper sx={{ p: 3, mb: 3 }}>
      <Typography variant="h6" fontWeight="bold" mb={1}>Mark Unavailability</Typography>
      <TextField label="Domain" value={domain} onChange={e => setDomain(e.target.value)} fullWidth sx={{ mb: 1 }} />
      <TextField label="From" type="date" value={start} onChange={e => setStart(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} sx={{ mb: 1 }} />
      <TextField label="To" type="date" value={end} onChange={e => setEnd(e.target.value)} fullWidth InputLabelProps={{ shrink: true }} sx={{ mb: 1 }} />
      <TextField label="Reason" value={reason} onChange={e => setReason(e.target.value)} fullWidth sx={{ mb: 1 }} />
      <Button onClick={submitUnavailability} variant="contained" color="primary">Submit</Button>
...
function TrainerUnavailabilityManagerDashboard() {
  ...
  async function handleViewAvailable(req) {
    ...
    try {
      ...
      alert("Failed to fetch available trainers. " + (error.response?.data?.error || error.message));
      setAvailableTrainers([]);
    }
  }
  ...
  return (
    <Paper sx={{ p: 3, mb: 3 }}>
      <Typography variant="h6" fontWeight="bold" mb={1}>Unavailability Requests</Typography>
          ...
          <TableBody>
            {unavail.map((req) => (
              <TableRow key={req.id}>
                ...
                <TableCell>
                  <Button variant="outlined" onClick={() => handleViewAvailable(req)}>List Trainers</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      {selectedReq && availableTrainers.length > 0 && (
        <Box>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>Available trainers for domain "{selectedReq.domain}"</Typography>
          <TableContainer>
            <Table size="small">
              <TableHead>
                <TableRow>
                ...
                <TableRow key={tr.email}>
                  ...
                  <TableCell>
                    {(tr.assignments || []).map((a, i) => (
                      <div key={i}>{a.batch_no} | {a.topic_name} | {a.start_time}-{a.end_time}</div>
                    ))}
                  </TableCell>
                  <TableCell>
                    {(tr.assignments || []).map((a, i) => (
          ...
          {assignMsg && <Alert severity="success" sx={{ mt: 1 }}>{assignMsg}</Alert>}
        </Box>
      )}
    </Paper>
  );
...
function TrainerDashboard({ user, token }) {
  ...
  // IMPORTANT: only a confirmed 'Completed' status should freeze actions.
  // This ensures selecting "Completed" will still show the tick (for confirmation),
  // and freezing happens only after the tick confirms (the backend update).
  const isActionFrozen = (topic) => topic.topic_status === "Completed";

  const groupedByModule = {};
  const moduleDates = {};
  topics.forEach((topic) => {
    const modName = topic.module_name || "(No Module)";
    if (!groupedByModule[modName]) {
      groupedByModule[modName] = [];
      moduleDates[modName] = topic.date || null;
    } else if (
      topic.date &&
      (!moduleDates[modName] || new Date(topic.date) < new Date(moduleDates[modName]))
    ) {
      moduleDates[modName] = topic.date;
    }
    groupedByModule[modName].push(topic);
  });

  const moduleNamesSortedByDate = Object.keys(groupedByModule).sort((a, b) => {
    const dateA = moduleDates[a] ? new Date(moduleDates[a]) : new Date(0);
    const dateB = moduleDates[b] ? new Date(moduleDates[b]) : new Date(0);
    return dateA - dateB || a.localeCompare(b);
  });

  const earliestIncompleteModuleIndex = (() => {
    for (let i = 0; i < moduleNamesSortedByDate.length; i++) {
      const modName = moduleNamesSortedByDate[i];
      const topicsInMod = groupedByModule[modName];
      for (const t of topicsInMod) {
        if (getStatusForTopic(t.id, t.topic_status) !== "Completed")
          return i;
      }
    }
    return -1;
  ...
  const isModuleEditable = (index) =>
    earliestIncompleteModuleIndex === -1 || index <= earliestIncompleteModuleIndex;
  ...
  async function handleStatusConfirm(topicId) {
    ...
    try {
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const res = await axios.post(`${API_BASE}/api/update-topic-status`, {
        topic_id: topicId,
        status: newStatus
      }, { headers });
      ...
      if (res.data && (res.data.success || res.status === 200)) {
        ...
        setPendingStatusChanges(prev => {
          const copy = { ...prev }; delete copy[topicId]; return copy;
        });
  ...
  async function handleRemarksSave(topicId, value) {
    try {
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const res = await axios.post(`${API_BASE}/api/update-remarks`, {
        topic_id: topicId,
        remarks: value
      }, { headers });
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: "auto", my: 3, fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif" }}>
      ...
      {tab === 0 && (
        <Paper elevation={6} sx={{ p: 4, borderRadius: 3, mb: 4, backgroundColor: "#ffffffcc" }}>
          ...
          <Typography variant="subtitle1" color="text.secondary" mb={3}>
            Welcome, <Box component="span" sx={{ fontWeight: "medium", color: "primary.main" }}>{welcomeName}</Box>!
          </Typography>
          ...
          {moduleNamesSortedByDate.map((moduleName, moduleIdx) => {
            const moduleTopics = groupedByModule[moduleName];
            const editable = isModuleEditable(moduleIdx);
            return (
              <Box key={moduleName} sx={{ mb: 5, boxShadow: 3, borderRadius: 3, backgroundColor: "#fefefe", p: 2 }}>
                <Typography
                  variant="h6"
                  sx={{
                    mb: 2,
                ...
                <TableContainer>
                  <Table size="small" sx={{ borderRadius: 2 }}>
                    <TableHead>
                      <TableRow sx={{ bgcolor: "#1976d2", "& th": { color: "white", fontWeight: "bold" } }}>
                    ...
                    <TableBody>
                      {moduleTopics.map((t) => {
                        ...
                        const currentStatus = pendingStatusChanges[t.id] ?? t.topic_status;
                        const frozen = isActionFrozen(t); // now frozen only if topic.topic_status === "Completed"
                        return (
                          <TableRow
                            key={t.id}
                            sx={{
                          ...
                          >
                            <TableCell sx={{ fontWeight: "600", color: "#1976d2", letterSpacing: 0.4 }}>
                            ...
                            <TableCell align="center">
                              <TextField
                                ...
                                FormHelperTextProps={{ sx: { fontStyle: "italic", fontSize: 10, color: grey[600] } }}
                                disabled={frozen}
                              />
                            </TableCell>
                            <TableCell align="center">
                              {daysDiff !== 0 ? (
                                <Chip
                                  ...
                                  sx={{
                                    fontWeight: "600",
                                    bgcolor:
                                      daysDiff > 2 ? red[100] : daysDiff > 0 ? orange[100] : green[100],
                                    color:
                                      daysDiff > 2 ? red[700] : daysDiff > 0 ? orange[700] : green[700],
                                  }}
                                />
                              ) : (
                            ...
                            <TableCell align="center">
                              <Chip
                                ...
                                sx={{
                                  fontWeight: "bold",
                                  bgcolor: statusChipColor[t.topic_status] ?? grey[300],
                                  color: t.topic_status === "Planned" ? grey[900] : "white",
                                  letterSpacing: 0.5,
                                  px: 1.5,
                                }}
                              />
                            ...
                            >
                              <Tooltip title={
                                frozen
                                  ...
                                  : editable
                                    ? "Change Status"
                                    : "Complete previous modules to edit"
                              }>
                                <span>
                                  <FormControl size="small" sx={{ minWidth: 140 }}>
                                    <Select
                                      value={currentStatus}
                                      disabled={!editable || !!t._pending || frozen}
                                      onChange={(e) =>
                                        handlePendingStatusChange(t.id, e.target.value)
                                      }
                                      sx={{
                                        backgroundColor: editable && !frozen ? "#e3f2fd" : "#f5f5f5",
                                        color: editable && !frozen ? "#0d47a1" : grey[600],
                                        fontWeight: "600",
                                      }}
                                    >
                              ...
                              {/* Show confirm tick when there's a pending change different from confirmed status.
                                  Note: we don't suppress the tick for "Completed" selection — the tick appears,
                                  and after confirming, the topic becomes Completed and the row freezes. */}
                              {pendingStatusChanges[t.id] &&
                                pendingStatusChanges[t.id] !== t.topic_status &&
                                !t._pending && (
          ...
          {message && (
            <Fade in={!!message}>
              <Box mt={1}>
                <Alert severity={message.startsWith("✅") ? "success" : "warning"} sx={{ fontWeight: "medium" }}>
                  {message}
                </Alert>
              </Box>
            </Fade>
      ...
      {tab === 1 && (
        <Box>
          {user?.role === "trainer" && <TrainerUnavailabilityForm user={user} />}
          {(user?.role === "manager" || user?.role === "admin") && (
            <TrainerUnavailabilityManagerDashboard />
          )}
          {!["trainer", "manager", "admin"].includes(user?.role) && (
            <Alert severity="warning" sx={{ mt: 3 }}>
              Trainer Management is only available to trainers, managers, or admins. Please ensure your account has the correct role.
            </Alert>
          )}
        </Box>
      )}
