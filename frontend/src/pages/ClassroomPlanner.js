...
function parseClassRoomColumn(value) {
  ...
  let slot = null;
  if (match[2]) slot = match[2] === "1" ? "morning" : "evening";
  else if (match[3]) slot = match[3].toLowerCase();
...
function parseExcelDate(value) {
  ...
  if (/^\d{2}\.\d{2}\.\d{4}$/.test(value)) { const [dd,mm,yyyy] = value.split("."); return new Date(`${yyyy}-${mm}-${dd}`); }
...
function isDateOverlap(s1,e1,s2,e2) {
  const a=new Date(s1),b=new Date(e1),c=new Date(s2),d=new Date(e2);
  if(isNaN(a)||isNaN(b)||isNaN(c)||isNaN(d)) return true;
  return a<=d && c<=b;
}

function normalizeRowKeys(row) { const o={}; Object.keys(row).forEach(k=>{o[k.trim()]=row[k];}); return o; }
...
function planClassroomsForOffline(rows) {
  const classrooms=[{name:"Ganga",capacity:50},{name:"Yamuna",capacity:35},{name:"Cauvery",capacity:35}];
  const shifts=["morning","evening"], plans=[], unallocated=[], occupancyIndex={};
  const filteredRows=rows.map(normalizeRowKeys).filter(row=>{
  ...
  filteredRows.sort((a,b)=>{
    ...
    const aO=computeOccupancyEnd(parseExcelDate(a["A.START DATE"])), bO=computeOccupancyEnd(parseExcelDate(b["A.START DATE"]));
    const endDiff=(aO?aO.getTime():0)-(bO?bO.getTime():0);
    if(endDiff!==0) return endDiff;
    return (a["COURSE"]||"").localeCompare(b["COURSE"]||"");
  });
  filteredRows.forEach(row=>{
    const course=row["COURSE"], startDt=parseExcelDate(row["A.START DATE"]);
    const aStart=toIsoDateString(startDt), aEnd=toIsoDateString(parseExcelDate(row["A.DUE DATE"]));
    const occEndDt=computeOccupancyEnd(startDt), occEnd=toIsoDateString(occEndDt);
    const enrolled=Number(row["ENROLLED"]||0), capacity=Number(row["CAPACITY"]||0);
    const classRoomRaw=row["CLASS_ROOM"]||row["CLASSROOM"]||"";
    const forcedAssignment=parseClassRoomColumn(classRoomRaw);
    if(enrolled>capacity){
      plans.push({batch_no:course,mode:"OFFLINE",a_start:aStart,a_end:aEnd,occupancy_end:occEnd,enrolled,capacity,classroom_name:"",slot:"",isAllocated:false,trainer_name:"UNASSIGNED",module_trainers:[],forced_classroom:forcedAssignment?`${forcedAssignment.roomName}_${forcedAssignment.slot}`:""});
      unallocated.push({batch_no:course,enrolled,a_start:aStart,a_end:aEnd,occupancy_end:occEnd}); return;
    }
    let allocated=false, assignedRoom="", assignedSlot="";
    if(forcedAssignment){
      const roomDef=classrooms.find(c=>c.name.toLowerCase()===forcedAssignment.roomName.toLowerCase());
      const roomLabel=roomDef?`${roomDef.name} [${roomDef.capacity}]`:`${forcedAssignment.roomName}`;
      const key=`${forcedAssignment.roomName}|${forcedAssignment.slot}`;
      if(!occupancyIndex[key]) occupancyIndex[key]=[];
      assignedRoom=roomLabel; assignedSlot=forcedAssignment.slot;
      occupancyIndex[key].push({start:aStart,end:occEnd,course}); allocated=true;
    } else {
      for(const room of classrooms){
        ...
        for(const slot of shifts){
          ...
          if(!occupancyIndex[key]) occupancyIndex[key]=[];
          const overlap=occupancyIndex[key].some(b=>isDateOverlap(aStart,occEnd,b.start,b.end));
          if(!overlap){
            assignedRoom=`${room.name} [${room.capacity}]`; assignedSlot=slot;
            occupancyIndex[key].push({start:aStart,end:occEnd,course}); allocated=true; break;
          }
        }
        if(allocated) break;
      }
    }
    if(!allocated) unallocated.push({batch_no:course,enrolled,a_start:aStart,a_end:aEnd,occupancy_end:occEnd});
    plans.push({batch_no:course,mode:"OFFLINE",a_start:aStart,a_end:aEnd,occupancy_end:occEnd,enrolled,capacity,classroom_name:assignedRoom,slot:assignedSlot,isAllocated:allocated,trainer_name:"UNASSIGNED",module_trainers:[],forced_classroom:forcedAssignment?`${forcedAssignment.roomName}_${forcedAssignment.slot}`:""});
  });
...
function getBatchColorMap(allMatrixTable) {
  ...
  allMatrixTable.forEach(row=>row.forEach(cell=>{if(Array.isArray(cell)) cell.forEach(bn=>batchSet.add(bn));}));
  const map={};
...
function normalizeLoadedRow(r, moduleTrainers, primaryTrainer) {
  const occEnd=r.occupancy_end||toIsoDateString(computeOccupancyEnd(parseExcelDate(r.occupancy_start)));
  return {batch_no:r.batch_no,classroom_name:r.classroom_name||"",slot:r.slot||"",a_start:r.occupancy_start||"",a_end:r.a_end||"",occupancy_end:occEnd,enrolled:r.enrolled||0,capacity:r.capacity||r.enrolled||0,mode:"OFFLINE",trainer_name:primaryTrainer,module_trainers:moduleTrainers,forced_classroom:r.forced_classroom||""};
}
...
export default function ClassroomPlanner() {
  ...
  const computeAndSetWeeks = useCallback((normalizedPlans) => {
    ...
    if(allDates.length) setWeeks(getWeeksInRange(allDates.reduce((a,b)=>a<b?a:b),allDates.reduce((a,b)=>a>b?a:b)));
    else setWeeks([]);
  },[]);
  ...
  const applyLoadedData = useCallback((occupancyRows, moduleTrainerMap, savedOverlapInfo) => {
    ...
    setUnallocatedBatches(unallocated.map(p=>({batch_no:p.batch_no,enrolled:p.enrolled,a_start:p.a_start,a_end:p.a_end,occupancy_end:p.occupancy_end})));
  ...
  const loadExistingMatrix = useCallback(async(silent=false)=>{
    try{
      ...
      if(!res.ok){if(!silent){setProcessingStatus("Failed to load saved data.");setPlans([]);setWeeks([]);setUnallocatedBatches([]);setTrainerOverlapInfo({});}return;}
      const data=await res.json();
      const{occupancyRows}=data||{};
      if(!occupancyRows?.length){if(!silent){setProcessingStatus("No saved data found.");setPlans([]);setWeeks([]);setUnallocatedBatches([]);setTrainerOverlapInfo({});}return;}
      const batchNos=occupancyRows.map(r=>r.batch_no).filter(Boolean);
      let moduleTrainerMap={},savedOverlapInfo={};
      try{
        const mtRes=await fetch(`${API_BASE}/api/get-batch-module-trainers`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({batch_nos:batchNos})});
  ...
  useEffect(()=>{
    const loadLicenses=async()=>{
      try{const res=await fetch(`${API_BASE}/api/licenses`);if(!res.ok)return;const data=await res.json();setLicenses(Array.isArray(data)?data:data.licenses||[]);}
      catch(e){setLicenseError("Failed to load licenses.");}
    };
    loadExistingMatrix(false);loadLicenses();
  },[loadExistingMatrix]);
    ...
    classrooms.forEach(room=>{
      ...
      roomSlots.forEach(slot=>{
        ...
        weeks.forEach(week=>{
          ...
          let batches=[];
          if(room==="UNALLOCATED") batches=unallocatedBatches.filter(p=>isDateOverlap(p.a_start,p.occupancy_end||p.a_end,si,ei)).map(p=>p.batch_no);
          else batches=plans.filter(p=>p.classroom_name===room&&p.slot===slot&&isDateOverlap(p.a_start,p.occupancy_end||p.a_end,si,ei)).map(p=>p.batch_no);
  ...
  const trainers=useMemo(()=>{
    ...
    plans.forEach(p=>{
      if(p.module_trainers?.length) p.module_trainers.forEach(mt=>{if(mt.trainer_name&&mt.trainer_name!=="UNASSIGNED")unique.add(mt.trainer_name);else hasUnassigned=true;});
    ...
    return trainers.map(trainer=>{
      ...
      weeks.forEach(week=>{
        ...
        const batches=plans.filter(p=>{
          if(!isDateOverlap(p.a_start,p.occupancy_end||p.a_end,si,ei)) return false;
  ...
  const fetchModuleTrainersForBatches=async(batchNos,offlinePlans)=>{
    try{
      ...
      (offlinePlans||[]).forEach(p=>{if(p.batch_no&&p.a_start&&p.occupancy_end) batch_date_ranges[p.batch_no]={start:p.a_start,end:p.occupancy_end};});
      const res=await fetch(`${API_BASE}/api/assign-batch-trainers-by-module`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({batch_nos:batchNos,batch_date_ranges})});
  ...
  const handleFileUpload=async(e)=>{
    ...
    try{
      ...
      const enrichedPlans=offlinePlans.map(p=>{
        ...
        return {...p,trainer_name:moduleTrainers.find(mt=>mt.trainer_name&&mt.trainer_name!=="UNASSIGNED")?.trainer_name||"UNASSIGNED",module_trainers:moduleTrainers};
      });
      ...
      setUnallocatedBatches([...unallocated].sort((a,b)=>(a.a_start||"").localeCompare(b.a_start||"")).map(u=>({batch_no:u.batch_no,enrolled:u.enrolled,a_start:u.a_start,a_end:u.a_end,occupancy_end:u.occupancy_end})));
      if(!offlinePlans.length){setError("No OFFLINE batches found in the file.");}
      else{
        ...
        setProcessingStatus(`Done! ${forcedCount} forced + ${autoCount} auto-allocated batches.`);
      }
    }catch(err){setError(`Failed to process file: ${err.message||"Invalid file format"}`);}
  ...
  const handleDownloadXlsx=async()=>{
    ...
    try{
      ...
      mh.font={bold:true,color:{argb:"FFFFFFFF"}};mh.fill={type:"pattern",pattern:"solid",fgColor:{argb:"FF333333"}};mh.alignment={horizontal:"center",vertical:"center",wrapText:true};
      table.forEach(row=>{
        ...
        row.forEach((cell,colIdx)=>{
          if(colIdx>=2&&Array.isArray(cell)&&cell.length>0){
            ...
            if(hexColor){
              ...
              ec.fill={type:"pattern",pattern:"solid",fgColor:{argb}};ec.font={bold:true,color:{argb:"FF222222"}};ec.alignment={horizontal:"center",vertical:"center",wrapText:true};
            }
          }
        });
        excelRow.getCell(1).font={bold:true};excelRow.getCell(2).alignment={horizontal:"center",vertical:"center"};
      });
  ...
  const handleSaveMatrix=async()=>{
    ...
    try{
      const allRows=[...plans,...unallocatedBatches.map(u=>({batch_no:u.batch_no,classroom_name:null,slot:null,a_start:u.a_start,a_end:u.a_end,occupancy_end:u.occupancy_end,enrolled:u.enrolled,capacity:u.capacity||0,trainer_name:null,module_trainers:[],forced_classroom:""}))].sort((a,b)=>{
        const sa=a.a_start||"",sb=b.a_start||"";if(sa!==sb)return sa.localeCompare(sb);
        const ea=a.occupancy_end||a.a_end||"",eb=b.occupancy_end||b.a_end||"";if(ea!==eb)return ea.localeCompare(eb);
        return (a.batch_no||"").localeCompare(b.batch_no||"");
      });
      const occupancyRows=allRows.map(p=>({batch_no:p.batch_no?.trim(),classroom_name:p.classroom_name||null,slot:p.slot||null,occupancy_start:p.a_start,occupancy_end:p.occupancy_end||p.a_end||null,enrolled:p.enrolled||0,capacity:p.capacity||p.enrolled||0,trainer_name:p.trainer_name||null,module_trainers:p.module_trainers||[],overlap_info:trainerOverlapInfo[p.batch_no]||[],forced_classroom:p.forced_classroom||null}));
      const res=await fetch(`${API_BASE}/api/save-classroom-matrix`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({occupancyRows})});
      const data=await res.json();
  ...
  const getWorstLicenseShortfall=(licenseInfo=[])=>licenseInfo.reduce((max,cur)=>cur.additional_needed>(max?.additional_needed||0)?cur:max,{additional_needed:0});
  ...
  return (
    <Box sx={{ minHeight:"100vh", background:TOKENS.bg, p:{xs:2,md:4}, fontFamily:"'DM Sans', sans-serif" }}>
      ...
      <Box sx={{ maxWidth:"98vw", mx:"auto" }}>
        {/* ── Page Header ── */}
        <Box sx={{ mb:4 }}>
          <Typography sx={{ fontFamily:"'DM Sans', sans-serif", fontSize:{xs:24,md:30}, fontWeight:800, color:TOKENS.text, letterSpacing:"-0.03em", mb:0.5 }}>Classroom Planner</Typography>
          <Typography sx={{ fontFamily:"'DM Sans', sans-serif", fontSize:14, color:TOKENS.textSub }}>Allocate offline batches to classrooms and manage trainer schedules</Typography>
        </Box>
              ...
              <Box sx={{ mt:3 }}>
                {selectedBatch && (
                  <Box sx={{ ...cardSx, border:`1.5px solid ${TOKENS.accent}44` }}>
                    <Box sx={{ px:3, py:2, background:TOKENS.accentLight, borderBottom:`1px solid ${TOKENS.border}`, display:"flex", alignItems:"center", gap:1 }}>
                      ...
                      {selectedBatch.forced_classroom && <Chip label={`📌 Forced: ${selectedBatch.forced_classroom}`} size="small" sx={{ ml:1, fontFamily:"'DM Sans', sans-serif", fontWeight:700, fontSize:11, background:TOKENS.warning.light, color:TOKENS.warning.text, border:`1px solid ${TOKENS.warning.fill}44` }}/>}
                    </Box>
                      ...
                      <Box sx={{ px:3, pb:3 }}>
                        ...
                        <TableContainer sx={{ borderRadius:"10px", border:`1px solid ${TOKENS.border}`, maxWidth:520 }}>
                          <Table size="small">
                            <TableHead>
                              <TableRow>
                                {["Module Type","Module Name","Trainer"].map(h=><TableCell key={h} sx={tableHeadSx}>{h}</TableCell>)}
                              </TableRow>
                            </TableHead>
                            <TableBody>
        ...
        <Box sx={{ ...cardSx, mb:3 }}>
          <SectionHeader
            ...
            subtitle={plans.length ? `${plans.length} batches · occupancy = start + 5 months + 2 weeks` : ""}
          ...
          <Box sx={{ p:3 }}>
            {loading ? (
              <Box sx={{ display:"flex", justifyContent:"center", alignItems:"center", height:160, gap:2 }}>
                <CircularProgress sx={{color:TOKENS.accent}}/><Typography sx={{ fontFamily:"'DM Sans', sans-serif", fontSize:13, color:TOKENS.textSub }}>Loading matrix...</Typography>
              </Box>
                            ...
                            <TableCell key={jdx} sx={{ minWidth:80, p:0.5, textAlign:"center", borderBottom:`1px solid ${TOKENS.border}` }}>
                              <Box sx={{ display:"flex", flexDirection:"column", alignItems:"center", gap:0.3 }}>
                                {Array.isArray(cell)&&cell.filter(Boolean).map((batch,bid)=>{
                                  ...
                                  return(
                                    <Chip key={bid} label={isForced?`📌 ${batch}`:batch} size="small"
                                      sx={{ backgroundColor:batchColorMap[batch]||"#e0e0e0", color:"#222", fontWeight:700, height:22, fontSize:"0.72rem", cursor:"pointer", border:isForced?"1.5px solid #f57c00":"none", fontFamily:"'DM Mono', monospace" }}
        ...
        {unallocatedBatches.length > 0 && (
          <Box sx={{ ...cardSx, mb:3, border:`1.5px solid ${TOKENS.error.fill}44` }}>
            <SectionHeader icon={<WarningAmberIcon sx={{fontSize:20,color:TOKENS.error.fill}}/>} title="Unallocated Batches" subtitle={`${unallocatedBatches.length} batch${unallocatedBatches.length>1?"es":""} could not be assigned`}/>
            <Box sx={{ p:3 }}>
              <TableContainer sx={{ borderRadius:"10px", border:`1px solid ${TOKENS.border}` }}>
                <Table size="small">
                  <TableHead><TableRow>{["Batch","Enrolled","Start Date","Due Date","Occupancy End (5m+2w)"].map(h=><TableCell key={h} sx={tableHeadSx}>{h}</TableCell>)}</TableRow></TableHead>
                  <TableBody>
                    {[...unallocatedBatches].sort((a,b)=>(a.a_start||"").localeCompare(b.a_start||"")).map(u=>(
                      <TableRow key={u.batch_no} sx={{background:TOKENS.error.light}}>
                        <TableCell sx={{ ...tableCellSx, fontWeight:700, color:TOKENS.error.text }}>{u.batch_no}</TableCell>
                        <TableCell sx={tableCellSx}>{u.enrolled}</TableCell>
                        <TableCell sx={tableCellSx}>{u.a_start}</TableCell>
                        <TableCell sx={tableCellSx}>{u.a_end}</TableCell>
        ...
        {plans.length > 0 && (
          <Box sx={{ ...cardSx }}>
            ...
            <Box sx={{ p:3 }}>
              {(()=>{
                const issues=plans.filter(p=>p.batch_no).map(p=>({batch_no:p.batch_no,shortages:getLicenseInfoForBatch(p.batch_no,p.capacity,p.enrolled).filter(l=>l.additional_needed>0)})).filter(p=>p.shortages.length>0);
